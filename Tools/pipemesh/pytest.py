...
def test1():
    ...
    network = wrapper.pipes.Network(
        1, 0.25, [1, 0, 0], 0.1
    )
    wrapper.gmsh.model.mesh.generate(3)
    n_nodes = len(wrapper.gmsh.model.mesh.getNodes()[0])
...
def test3():
    ...
    wrapper.gmsh.model.mesh.generate(3)
    n_nodes_before = len(wrapper.gmsh.model.mesh.getNodes()[0])
    network.generate(filename="test")
    with open("test.msh", 'r') as testFile:
        content = testFile.readlines()
